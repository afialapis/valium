...
const useValiumInput = (props) => {
  ...
  useEffect(() => {
    if (process.env.NODE_ENV !== "production") {
      if (inputRef!=undefined && inputRef.current!=undefined) {
        ...
        log('input', `${input.name} (${input.type}) running checkprops effect on useValiumInput  -- ${JSON.stringify([doRepeat, doNotRepeat, inputFilter, prematureValidation])}`)
  ...
  useEffect(() => {
    if (inputRef!=undefined && inputRef.current!=undefined) {
      ...
      log('input', `${input.name} (${input.type}) running validity effect on useValiumInput - ${JSON.stringify([prematureValidation, inputFilter, setValidity])}`)
