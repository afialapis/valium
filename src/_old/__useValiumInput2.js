...
const useValiumInput = (props) => {
  ...
  useEffect(() => {
    if (inputRef!=undefined && inputRef.current!=undefined) {
      ...
      log('input', `${input.name} (${input.type}) #${input.id} useValiumInput useEffect(2) running validity effect`)
      ...
      const handleChange = (event) => {
        log('input', `${input.name} (${input.type}) #${input.id} useValiumInput useEffect(2) handleChange() event ${event.type} value ${event.target.value} is calling setValidity`)
        setValidity()
      }
      ...
      const removeAllChangeListeners = () => {
        if (input!=undefined) {
          log('input', `${input.name} (${input.type}) #${input.id} useValiumInput useEffect(2) dettaching events`)
          Object.keys(allListeners).map((eventType) => {
            input.removeEventListener(eventType, allListeners[eventType])
          })
        } else {
          log('input', `${input.name} (${input.type}) #${input.id} useValiumInput useEffect(2) WARNING! Could not dettach events`)
        }
      }    
