...
const useValium = (props) => {
  ...
  const {formActions, inputType, checkValue, 
         allowedValues, disallowedValues, 
         doRepeat, doNotRepeat, stepRange, 
         inputFilter, prematureValidation, 
         feedback, bindSetValidity}= props

  const iconfig= getConfig(inputType)
      ...
      if (inputFilter!=undefined) {
        iconfig.input_filter_events.forEach(function(eventType) {
          const filterEventListener = function(event) {
            ...
            //      But detailed checks must be done and probably find a better solution, as probably this check
